...
class Auth extends CI_Controller 
{
	...
	private function _login()
	{
		...
		$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
  				<strong>Passwordnya Salah!</strong>
				</div>');
			redirect('auth');
				}
		...
		$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
  				<strong>Maaf!</strong> Email belum Aktif. Silahkan Aktivasi
				</div>');
			redirect('auth');
			}
	...
	public function registration()
	{
		...
		$user_token	= [
			...
			$this->session->set_flashdata('message','<div class="alert alert-success" role="alert">
  				<strong>Selamat!</strong> Silahkan cek Email untuk Aktivasi.
				</div>');
			redirect('auth');
	...
	private function _sendEmail($token, $type)
	{
		$config = [
				...
				'smtp_user'	=> 'farrasammar13@gmail.com',
				'smtp_pass' => '548034farrasai',
				'smtp_port' => 465,
				'mailtype' 	=> 'html',
				'charset'	=> 'utf-8',
				'newline'	=> "\r\n"
		...
		$this->email->from('farrasammar13@gmail.com','Farras Ammar Isnandy');
		$this->email->to($this->input->post('email'));


		if ($type == 'verify') {
		$this->email->subject('Verifikasi Akun');
		$this->email->message('Klik link ini untuk verifikasi akun : <a href="'. base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Aktivasi</a>');
	...
	public function verify()
	{
		...
		if($user) {
			...
			$this->session->set_flashdata('message','<div class="alert alert-success" role="alert">
  				<strong>'. $email .'</strong> sudah di aktivasi ! Silahkan Login.
				</div>');
			redirect('auth');
			...
			$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
  				<strong>Akun gagal Aktivasi!. Token Kadaluarsa</strong>
				</div>');
			redirect('auth');
		}
			...
			$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
  				<strong>Akun gagal Aktivasi!. Token Salah</strong>
				</div>');
			redirect('auth');
			}
			...
			$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
  				<strong>Akun gagal Aktivasi!. Email Salah</strong>
				</div>');
			redirect('auth');
		}
	}
	...
	public function logout()
	{
		...
		$this->session->set_flashdata('message','<div class="alert alert-success" role="alert">
  				<strong>Kamu Sudah Logout!</strong>
				</div>');
	...
	public function forgotPassword()
	{
		...
		$this->load->view('templates/auth_footer');
			...
			if($user){
				...
				$this->session->set_flashdata('message','<div class="alert alert-success" role="alert">
  				<strong>Cek Email untuk reset password!</strong>
				</div>');
			redirect('auth/forgotpassword');

			} else {
				$this->session->set_flashdata('message','<div class="alert alert-danger" role="alert">
  				<strong>email tidak terdaftar / belum aktivasi!</strong>
				</div>');
			redirect('auth/forgotpassword');
			}
		}
